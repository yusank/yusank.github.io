...
__文章原始地址:__ http://tonybai.com/2015/11/17/tcp-programming-in-golang/
...
我们通过`client2.go`发送”hi”到Server端：
运行结果:
...
有些场合对Read的阻塞时间有严格限制，在这种情况下，Read的行为到底是什么样的呢？在返回超时错误时，是否也同时Read了一部分数据了呢？这个实验比较难于模拟，下面的测试结果也未必能反映出所有可能结果。我们编写了`client4.go`和`server4.go`来模拟这一情形。
...
© 2015, [bigwhite](http://tonybai.com/). 版权所有.